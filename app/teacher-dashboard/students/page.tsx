...
export default function StudentsPage() {
  ...
  const groupedStudents = useMemo(() => {
    ...
    filtered.forEach((student) => {
      const section = "A" // Simplified section
      const key = `${student.department} • ${student.year} • Section ${section}`

      if (!groups[key]) {
        groups[key] = { key, department: student.department, year: student.year, section, students: [] }
      }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-muted-foreground">
          Showing {groupedStudents.reduce((total, group) => total + group.students.length, 0)} students in {groupedStudents.length} groups
        </p>
      ...
      <div className="space-y-4">
        {groupedStudents.length > 0 ? groupedStudents.map((group) => (
          <Card key={group.key}>
            <CardHeader className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => toggleGroup(group.key)}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {expandedGroups.has(group.key) ? <ChevronDown className="h-5 w-5 text-muted-foreground" /> : <ChevronRight className="h-5 w-5 text-muted-foreground" />}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-sm text-muted-foreground">
                    Avg: {Math.round(group.students.reduce((sum, s) => sum + s.attendancePercentage, 0) / group.students.length)}%
                  </div>
                </div>
              </div>
            </CardHeader>
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-4">
                            <div className="text-right">
                              ...
                              <p className={cn("font-bold", getAttendanceColor(student.attendancePercentage))}>{student.attendancePercentage}%</p>
                              <p className="text-xs text-muted-foreground">{student.attendedClasses}/{student.totalClasses} classes</p>
                            </div>
                        ...
                        <div className="mt-3 pt-3 border-t border-border">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                            <div className="flex items-center gap-2 text-muted-foreground"><Mail className="h-3 w-3" />{student.email}</div>
                            {student.phone && <div className="flex items-center gap-2 text-muted-foreground"><Phone className="h-3 w-3" />{student.phone}</div>}
                          </div>
                        </div>
                      </CardContent>
                    </Card>
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-muted-foreground">
                {students.length === 0 ? "No students have been approved yet. Check the Approvals section." : "Try adjusting your search criteria or filters."}
              </p>
            </CardContent>
          </Card>
        )}
